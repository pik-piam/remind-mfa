...
class PriceDrivenTrade(SimsonBaseModel):
    ...
    def compute_price_driven_trade(
        ...
        raise RuntimeError(
            "Could not converge to a solution for the price driven trade."
        )
    ...
    def calibrate(
        ...
        for i in range(self.max_iter):
            ...
            self.export_penalty += 1. - self.export_penalty.values.min()
    ...
    def get_trade(self, price: fd.FlodymArray, demand: fd.FlodymArray) -> tuple[fd.FlodymArray, fd.FlodymArray]:
    ...
    def origin_shares(self, price: fd.FlodymArray) -> fd.FlodymArray:
        local_price = self.price_cast(price) * self.export_penalty_cast() * self.domestic_preference_cast()
    ...
    def export_penalty_cast(self) -> fd.FlodymArray:
        ...
        cast_out.values[np.diag_indices(self.n_regi)] = 1.
        return cast_out
