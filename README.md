...
```bash
uv run scripts/copy_madrat_archive.py mfa-archive --config default <hpc>
```
where `<hpc>` is the ssh address/alias of the PIK cluster, e.g., `<Your PIK user name>@hpc.pik-potsdam.de`.
Alternatively, you can also use `uv run scripts/copy_madrat_archive.py mrmfa-sources <hpc>` to copy the input data for `mrmfa` from the cluster to your local machine, and then run `mrmfa` locally to generate the madrat output data.
