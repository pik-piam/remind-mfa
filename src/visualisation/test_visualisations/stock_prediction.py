...
def predict(histdata, satlevel, sattime):
    ...
    # calculate parameters for sigmoid predict
...
plt.stackplot(time, [np.divide(datadic['Transport'].s, totalPop), np.divide(datadic['Machinery'].s, totalPop),
                     np.divide(datadic['Construction'].s, totalPop), np.divide(datadic['Product'].s, totalPop)],
