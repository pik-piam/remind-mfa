...
def _(Path, load_dotenv, mo, os):
    ...
    if not remind_archives:
        raise FileNotFoundError(
            f"No *_remind.tgz archives found in {madrat_output_dir}. Run `uv run scripts/fetch_from_hpc.py remind-archive --config default <hpc>` to copy the archive from the HPC."
        )
